...
class LocationService: Service(), LocationListener, GpsStatus.Listener {
    ...
    private val ANDROID_CHANNEL_ID = "com.example.gpstracker"
    ...
    override fun onLocationChanged(newLocation: Location?) {
        newLocation?.let{
            Log.d(LOG_TAG, "(" + it.latitude + "," + it.longitude + ")") //現在地のログ出力
            ...
            LocalBroadcastManager.getInstance(this.application).sendBroadcast(intent)//位置情報のブロードキャスト
        }
    ...
    private fun filterAndAddLocation(location: Location): Boolean {
        ...
        if (predictedDeltaInMeters > 60) { //60m以上離れているときgpsを記録しない
