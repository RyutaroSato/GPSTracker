...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        locationUpdateReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                ...
                drawLocationAccuracyCircle(newLocation) //ユーザーの位置を円示す
                drawUserPositionMarker(newLocation) //ユーザーの位置をマーカで示す

                this@MainActivity.locationService?.let {
                    if (it.isLogging) {
                        addPolyline() //移動経度を線として描画
                    }
                }
    ...
    private fun zoomMapTo(location: Location) {
        ...
        if (this.didInitialZoom == false) { //アプリが起動後に一度でもユーザーの現在地にマップをズームしていない場合
            try {
        ...
        //ユーザーがマップに触ってから10秒間はfalseになるようになっており(setupgpooglemap関数内)、これがtrueの場合だけ自動でズームをする
        if (zoomable) {
            try {
                zoomable = false
    ...
    private fun drawLocationAccuracyCircle(location: Location) {
        ...
        locationAccuracyCircle?.let{
            ...
            //円の半径をaccuracyに設定し、円の大きさで緯度経度の正確さを見る
    ...
    //locationlistの最新の情報と一つ前の情報の2点間を直線で結ぶ
